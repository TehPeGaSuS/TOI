# Privacy Policy

**Effective date:** January 11, 2026  
**Website:** https://hookupz.win

Hookupz.win (“we”, “us”, “our”) operates an adult-oriented online chat platform intended **exclusively for users aged 18 years or older**. We take your privacy seriously and are committed to protecting your personal data in accordance with applicable privacy laws, including the **General Data Protection Regulation (GDPR)** and the **California Online Privacy Protection Act (CalOPPA)**.

By accessing or using Hookupz.win, you agree to the collection and use of information in accordance with this Privacy Policy.

---

## 1. Eligibility (18+ Only)

Hookupz.win is strictly intended for adults.  
By using this site, you confirm that:

- You are **at least 18 years old** (or the age of majority in your jurisdiction)
- You are legally permitted to access adult-oriented content

We do not knowingly collect personal data from minors. If you believe a minor has provided us with personal data, please contact us immediately so we can remove it.

---

## 2. Information We Collect

### 2.1 Information You Provide Voluntarily
When using Hookupz.win, you may provide:

- Username and display name
- Email address (if registration or account recovery is enabled)
- Profile information you choose to share
- Messages, files, images, or other content you voluntarily submit
- Audio and video data during voice or video calls

**Important:** Messages and shared content may be visible to other users depending on channel or room settings.

---

### 2.2 Automatically Collected Information
We may automatically collect certain technical data, including:

- IP address
- Browser type and version
- Operating system
- Date and time of access
- General usage and diagnostic data

This information is used for security, moderation, troubleshooting, and service improvement.

---

### 2.3 Cookies and Similar Technologies
Hookupz.win may use cookies or similar technologies for:

- Authentication and session management
- Security purposes
- Basic analytics and functionality

You can control or disable cookies via your browser settings. Some features may not function properly without cookies.

---

## 3. Voice and Video Communications

### 3.1 Voice Calls (Built-In)
Hookupz.win provides **built-in voice calling functionality** as part of the platform.

- Voice communications may be processed by the platform’s underlying software (Mattermost)
- Audio data is transmitted to facilitate real-time communication between users
- We do not actively record voice calls unless required for security, moderation, or legal compliance

Voice communications are subject to this Privacy Policy and applicable data protection laws.

---

### 3.2 Video Calls (Third-Party Services)
Video calling functionality on Hookupz.win is provided via **third-party services** integrated into the platform.

- Video and associated audio streams may be processed directly by third-party providers
- We do not control how third-party providers collect, store, or process data
- The privacy practices of these providers are governed by their own privacy policies

By using video call features, you acknowledge that your data may be processed by external services and agree to their applicable terms and privacy policies.

---

## 4. How We Use Your Information

We use collected information to:

- Provide and operate the service
- Maintain security and prevent abuse
- Enforce rules and terms of service
- Communicate important service-related information
- Improve platform functionality and performance
- Comply with legal obligations

We **do not sell** your personal data.

---

## 5. Legal Basis for Processing (GDPR)

Under GDPR, we process personal data based on:

- **Consent** (e.g., when you create an account or share content)
- **Contractual necessity** (to provide the service)
- **Legitimate interests** (security, abuse prevention, service improvement)
- **Legal obligations** (when required by law)

---

## 6. Data Retention

We retain personal data only for as long as necessary to:

- Provide the service
- Comply with legal obligations
- Resolve disputes
- Enforce agreements

Users may request account deletion, subject to legal and technical limitations.

---

## 7. Data Sharing and Disclosure

We may share data only:

- With service providers necessary to operate the platform
- To comply with legal obligations or lawful requests
- To protect the rights, safety, and security of users and the platform

We do **not** share personal data for marketing purposes.

---

## 8. International Data Transfers

Your data may be processed or stored outside your country of residence, including outside the European Economic Area (EEA).

When such transfers occur, we take reasonable measures to ensure appropriate safeguards are in place.

---

## 9. Your Rights (GDPR)

If you are located in the EU or EEA, you have the right to:

- Access your personal data
- Request correction of inaccurate data
- Request deletion of your data (“right to be forgotten”)
- Restrict or object to processing
- Request data portability
- Withdraw consent at any time

Requests can be made using the contact details below.

---

## 10. California Privacy Rights (CalOPPA & CCPA Notice)

For California residents:

- We disclose what personal information is collected and how it is used
- We do not sell personal information
- You may request access to or deletion of your personal data
- We do not discriminate against users who exercise privacy rights

---

## 11. Security

We implement reasonable technical and organisational measures to protect personal data.  
However, **no online service is 100% secure**, and we cannot guarantee absolute security.

Users are responsible for maintaining the confidentiality of their account credentials.

---

## 12. Links to Other Websites

Hookupz.win may contain links to third-party websites or services.  
We are not responsible for their content or privacy practices.

---

## 13. Changes to This Privacy Policy

We may update this Privacy Policy from time to time.  
Changes will be posted on this page with an updated effective date.

Continued use of the service constitutes acceptance of the updated policy.

---

## 14. Contact Information

For privacy-related questions, data requests, or concerns, please contact:

**Email:** zncuser@gmail.com

---

By using Hookupz.win, you acknowledge that you have read, understood, and agreed to this Privacy Policy.
